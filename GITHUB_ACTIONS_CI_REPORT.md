...
| `release` | GitHub Release 발행 시 실행 |
| `pull_request_review` | PR 리뷰 제출 시 실행 |
...
과제에서 "여러 이벤트 조합"이라는 표현이 있었기 때문에 단순히 `push`만 사용하는 것보다 여러 상황을 고려한 이벤트 구성이 더 적절했다. 이번 실습에서는 `push`, `pull_request`, `workflow_dispatch`에 더해 `schedule`, `release`, `pull_request_review`까지 함께 사용했다.
...
| `schedule` | 정기 점검 및 반복 검증 가능 |
| `release` | 릴리스 시점 기준 자동 실행 가능 |
| `pull_request_review` | 리뷰 제출 시 추가 검증 흐름 구성 가능 |
...
### 6-5. 이벤트를 어디까지 추가할지에 대한 판단

이벤트를 많이 넣을수록 워크플로 활용 범위는 넓어지지만, 반대로 너무 과도하게 설정하면 과제 설명이 복잡해질 수 있다. 이번 실습에서는 여러 이벤트 조합을 보여주기 위해 `schedule`, `release`, `pull_request_review`를 추가했지만, 각 이벤트의 의미를 명확히 설명할 수 있는 수준에서 구성하는 것이 중요했다.

즉, 이벤트 추가 자체보다도 "왜 이 이벤트를 넣었는가"를 설명할 수 있어야 좋은 구성이 된다는 점을 확인했다.
...
### 7-5. 추가 이벤트를 사용할 때 알면 좋은 점

| 이벤트 | 실무에서의 활용 예시 |
| --- | --- |
| `schedule` | 야간 빌드, 정기 점검, 주간 검증 |
| `release` | 버전 배포 시점 이미지 생성 또는 배포 자동화 |
| `pull_request_review` | 승인 또는 리뷰 제출 이후 추가 검증 흐름 연결 |
...
이번 실습을 통해 GitHub Actions를 사용하여 프론트엔드 프로젝트의 CI 파이프라인을 구성했다. 워크플로는 `push`, `pull_request`, `workflow_dispatch`, `schedule`, `release`, `pull_request_review` 이벤트를 기반으로 동작하며, 소스코드 checkout 이후 Node.js 환경 설정, 의존성 설치, 프론트엔드 빌드, Docker 이미지 빌드, Harbor Registry push 순으로 실행되도록 구성했다.
...
이번 실습에서는 GitHub Actions를 활용해 `소스코드 변경 -> 코드 빌드 -> Docker 이미지 빌드 -> Harbor Registry push` 흐름을 자동화하는 CI 파이프라인을 구성했고, 다양한 이벤트 조합과 단계 분리를 통해 과제 요구사항과 실무적 관점을 함께 반영했다.
